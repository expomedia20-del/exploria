...
class EcoParkDemoReadinessTest extends TestCase
{
    ...
    public function test_ecopark_pilot_seed_stays_blocked_until_sponsor_reward_is_approved(): void
    {
        $report = app(EcoParkDemoReadinessService::class)->report();

        $this->assertFalse($report['summary']['ready']);
        $this->assertSame(1, $report['summary']['failCount']);
        ...
        $this->assertSame('fail', $checks['sponsor_rewards']['status']);
