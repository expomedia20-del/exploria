...
function itemMeta(item: JourneyItem | Participant) {
    ...
    if (journeyItem.type === 'mission') {
        return `${journeyItem.points ?? 0} امتیاز`;
    }

    if (journeyItem.rewardType) {
        return journeyItem.rewardType;
    }

    if (journeyItem.treasureType) {
        return journeyItem.treasureType;
    }

    if (journeyItem.adType) {
        return journeyItem.adType;
    }

    if (journeyItem.deviceType) {
        return journeyItem.deviceType;
    }
...
function itemHref(item: JourneyItem | Participant) {
    ...
    if (journeyItem.type === 'qr') {
        return '/admin/qr-codes';
    }

    if (
        journeyItem.type === 'mission' ||
        journeyItem.type === 'reward' ||
        journeyItem.type === 'treasure'
    ) {
    ...
    if (journeyItem.type === 'display') {
        return '/admin/display-operations';
    }
...
function itemKind(item: JourneyItem | Participant) {
    if ('participationRole' in item) {
        return label(typeLabels, item.participantType);
    }
    ...
    if (journeyItem.type === 'reward') {
        return 'پاداش';
    }

    if (journeyItem.type === 'treasure') {
        return 'گنج';
    }
...
function detailRows(
    ...
    if ('participationRole' in item) {
        return [
            ...
            [
                'اتصال ها',
                `${fa(item.connections.rewards)} پاداش، ${fa(item.connections.ads)} تبلیغ، ${fa(item.connections.missions)} ماموریت`,
            ],
        ];
    }
...
function participantNames(participants: Participant[]) {
    const names = participants
        .map((participant) => participant.partner?.name)
        .filter(Boolean);
...
function itemOperationalGuide(
    ...
    if ('participationRole' in item) {
        return {
            ...
            navigation: [
                item.hub?.name
                    ? `این عضو در محدوده ${item.hub.name} دیده می شود.`
                    : 'این عضو به هاب مشخصی وصل نشده و باید جایگاه مکانی آن تعیین شود.',
    ...
    if (journeyItem.type === 'mission') {
        return {
            ...
            navigation: [
                hub
                    ? `کاربر باید به محدوده ${hub} هدایت شود.`
                    : 'برای این مأموریت هنوز هاب یا نقطه مکانی مشخص نشده است.',
    ...
    if (journeyItem.type === 'ad') {
        return {
            ...
            navigation: [
                hub
                    ? `این تبلیغ در محدوده ${hub} معنا پیدا می کند.`
                    : 'برای تبلیغ هنوز محدوده مکانی مشخص نشده است.',
    ...
    if (journeyItem.type === 'display') {
        return {
            ...
            navigation: [
                hub
                    ? `نمایشگر به محدوده ${hub} وابسته است.`
                    : 'برای نمایشگر هنوز هاب یا نقطه نصب مشخص نشده است.',
...
function hubOperationalGuide(
    ...
    return {
        ...
        navigation: [
            group.hub
                ? `این گروه در نقشه مکانی باید زیر محدوده ${hubName} نمایش داده شود.`
                : 'این گروه باید قبل از اجرا به یک محدوده مکانی یا مسیر خارجی وصل شود.',
            `در ${campaign.name} مسیر کاربر باید نشان دهد از کدام QR وارد این هاب می شود و بعد به کدام نقطه می رود.`,
        ],
    };
}

function sponsorOperationalGuide(
    type: 'internal' | 'external',
    sponsors: Participant[],
    campaign: CampaignBlueprint,
): OperationGuide {
    ...
    return {
        ...
        navigation: [
            ...
            sponsors.length === 0
                ? 'هنوز اسپانسری ثبت نشده؛ بعد از ثبت، نقطه اثرگذاری آن در مسیر کمپین مشخص می شود.'
                : 'برای هر اسپانسر ثبت شده، نقطه تماس با کاربر یا گزارش تبلیغاتی را مشخص کنید.',
        ],
    };
}
...
function OperationDetailsSheet({
    ...
    const rows = selection
        ? (selection.rows ??
          (selection.item
              ? detailRows(selection.item, selection.campaign)
              : []))
        : [];
                    ...
                    <>
                        <SheetHeader>
                            <SheetTitle>
                                {selection.title ??
                                    (selection.item
                                        ? itemTitle(selection.item)
                                        : selection.sectionTitle)}
                            </SheetTitle>
                            <SheetDescription>
                                {selection.sectionTitle} در{' '}
                        ...
                        <div className="space-y-3 px-4">
                            ...
                            <div className="divide-y divide-sidebar-border/70 rounded-lg border border-sidebar-border/70 dark:divide-sidebar-border dark:border-sidebar-border">
                                {rows.map(([title, value]) => (
                                    <div
                                        key={title}
                                        className="grid grid-cols-[0.85fr_1.15fr] gap-3 px-3 py-2 text-sm"
                                    >
...
export default function CampaignOperationsIndex({
    stats,
    campaigns,
    selectedBlueprint,
    selectedCampaign,
    ...
    const activeCampaign = selectedCampaign
        ? campaigns.find((campaign) => campaign.id === selectedCampaign.id)
        : null;
    const activeAlignmentErrors =
        activeCampaign?.alignment.issues.filter(
            (issue) => issue.level === 'error',
        ) ?? [];
    const activeOperationalErrors =
        activeCampaign?.operationalReview.issues.filter(
            (issue) => issue.level === 'error',
        ) ?? [];
    const routeBlockers =
        activeAlignmentErrors.length + activeOperationalErrors.length;
                ...
                <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                    <div>
                        <p className="text-sm text-muted-foreground">
                            طرح یکپارچه بازی، بازدید و تبلیغات
                        </p>
                    ...
                    <section className="rounded-lg border border-primary/25 bg-primary/5 p-4 text-sm shadow-sm">
                        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                            <div>
                                ...
                                <p className="mt-1 text-muted-foreground">
                                    داده الگو آمده تا مسیر، QR، نقطه شروع و محل
                                    تحویل پاداش به نقشه عملیات وصل شود.
                                </p>
                            </div>
                            <span
                                className="rounded-full bg-background px-3 py-1 text-xs"
                        ...
                        <div className="mt-4 grid gap-3 lg:grid-cols-3">
                            <div className="rounded-lg bg-background/75 p-3">
                                <p className="font-medium">ناوبری و مسیر</p>
                                <p className="mt-1 text-muted-foreground">
                            ...
                            <div className="rounded-lg bg-background/75 p-3">
                                <p className="font-medium">
                                    چرخه و مأموریت‌های متناظر
                                </p>
                                <ol className="mt-2 space-y-1 text-muted-foreground">
                                    {(selectedBlueprint.missionPlan ?? [])
                                        .slice(0, 4)
                                        .map((step) => (
                    ...
                    <section className="exploria-panel">
                        <div className="border-b border-border/70 px-4 py-3 dark:border-sidebar-border">
                            <h2 className="font-semibold">
                                تایید مرحله ۵: مسیر عملیاتی کمپین
                            </h2>
                            <p className="mt-1 text-sm text-muted-foreground">
                                بعد از بررسی اتصال QR، مأموریت، مشوق، اعضا و
                                نمایشگرها، مسیر همین کمپین را تایید کنید.
                            </p>
                        </div>
                        <div className="grid gap-4 p-4 lg:grid-cols-[1fr_0.8fr]">
                            <div className="grid gap-3 text-sm sm:grid-cols-5">
                                {[
                                    ...
                                    [
                                        'مشوق',
                                        activeCampaign.stats.rewards +
                                            activeCampaign.stats.treasures,
                                    ],
                                    [
                                        'عضو آماده',
                                        activeCampaign.stats.readyParticipants,
                                    ],
                                    [
                                        'نمایش/تبلیغ',
                                        activeCampaign.stats.adRequests +
                                            activeCampaign.stats.displayDevices,
                                    ],
                                ].map(([labelText, value]) => (
                                    <div
                            ...
                            <div className="rounded-lg border border-border/80 bg-card/75 p-3 shadow-sm">
                                <p className="text-sm font-semibold">
                                    {activeCampaign.routeReviewedAt
                                        ? 'مسیر تایید شده است'
                                        : 'مسیر هنوز تایید نشده است'}
                                </p>
                                <p className="mt-1 text-xs text-muted-foreground">
                                    کنترل مسیر:{' '}
                                    {activeCampaign.operationalReview.status ===
                                    'ready'
                                        ? 'آماده تایید عملیاتی'
                                        : `${routeBlockers.toLocaleString('fa-IR')} مانع اصلی قبل از تایید`}
                                </p>
                                {activeCampaign.routeReviewedAt ? (
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        زمان تایید:{' '}
                                ...
                                <div className="mt-3 grid gap-2">
                                    {activeCampaign.operationalReview.checks.map(
                                        (check) => (
                                            <div
                                                key={check.key}
                                                className="flex items-start justify-between gap-3 rounded-md bg-muted/40 px-2 py-1.5 text-xs"
                                            >
                                                <div>
                                                    <p className="font-medium">
                                                        {check.title}
                                ...
                                {activeCampaign.operationalReview.issues
                                    .length > 0 ? (
                                        ...
                                        {({ processing }) => (
                                            <>
                                                ...
                                                <textarea
                                                    name="route_notes"
                                                    defaultValue={
                                                        activeCampaign.routeReviewNotes ??
                                                        ''
                                                    }
                ...
                {campaigns.length === 0 ? (
                    ...
                    campaigns.map((campaign) => (
                        <section key={campaign.id} className="exploria-panel">
                            <div className="flex flex-col gap-3 border-b border-border/70 p-4 lg:flex-row lg:items-start lg:justify-between dark:border-sidebar-border">
                                <div>
                            ...
                            <div className="border-b border-border/70 p-4 dark:border-sidebar-border">
                                <div className="mb-3 flex flex-col gap-1">
                                    <h3 className="font-semibold">
                                        خط زمان عملیاتی گام‌های چرخه
                                    </h3>
                                    <p className="text-sm text-muted-foreground">
                                        این نما نشان می‌دهد هر گام چرخه با کدام
                                        QR، مأموریت، پاداش/گنج و عضو آماده اجرا
                                        پشتیبانی می‌شود.
                                    </p>
                                </div>
                                            ...
                                            >
                                                    ...
                                                    <span
                                                        className={
                                                            ...
                                                            'ready'
                                                                ...
                                                                : 'rounded-full bg-amber-100 px-2.5 py-1 text-xs font-medium text-amber-800 dark:bg-amber-950 dark:text-amber-200'
                                                        }
                                                    >
                                                        {step.status === 'ready'
                                                            ? 'آماده اجرا'
                                                ...
                                                <div className="mt-3 grid gap-2 text-xs sm:grid-cols-4">
                                                    {step.checks.map(
                                                        (check) => (
                                                            <div
                                                                key={`${step.index}-${check.key}`}
                                                                className="rounded-md bg-muted/45 px-2.5 py-2"
                                                            >
                                                ...
                                                {step.pendingRewards > 0 ? (
                                                    <p className="mt-3 rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-900 dark:bg-amber-950/30 dark:text-amber-100">
                                                        ...
                                                        پیشنهاد پاداش این گام
                                                        هنوز در انتظار بازبینی
                                                        است.
                                                    </p>
                                                ) : null}
                                            </article>
                            ...
                            <div className="grid gap-4 p-4 xl:grid-cols-[1fr_0.72fr]">
                                ...
                                <aside className="space-y-4">
                                    <div className="rounded-lg border border-border/80 bg-card/75 p-3 shadow-sm">
                                        ...
                                        <div className="space-y-3">
                                            {campaign.participantsByHub.map(
                                                (group) => (
                                                    <button
                                                        key={
                                                            group.hub?.id ??
                                    ...
                                    <div className="rounded-lg border border-border/80 bg-card/75 p-3 shadow-sm">
                                        ...
                                        <div className="mt-3 space-y-2">
                                            {campaign.redemptionOverview.latest
                                                .length > 0 ? (
                                                campaign.redemptionOverview.latest.map(
                                                    (redemption) => (
                                                        <div
                                                            key={redemption.id}
                                                            className="rounded-md bg-background px-2.5 py-2 text-xs"
                                                        >
                                                            <div className="flex items-start justify-between gap-2">
                                                                ...
                                                                <span
                                                                    className={
                                                                        redemption.status ===
                                                                        'confirmed'
                                                                            ? 'shrink-0 rounded-full bg-emerald-100 px-2 py-0.5 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200'
                                                                            : 'shrink-0 rounded-full bg-amber-100 px-2 py-0.5 text-amber-900 dark:bg-amber-950 dark:text-amber-100'
                                                                    }
                                                                >
                                                                    {redemption.status ===
                                                                    'confirmed'
                                                ...
                                                <p className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
                                                    هنوز کد مصرفی برای پاداش‌های
                                                    این کمپین صادر نشده است.
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                                        <button
                                            ...
                                            onClick={() =>
                                                setSelectedOperation({
                                                    title: 'اسپانسر داخلی',
                                                    rows: sponsorDetailRows(
                                                        'internal',
                                                        campaign.sponsors
                                                            .internal,
                                                        campaign,
                                                    ),
                                                    guide: sponsorOperationalGuide(
                                                        'internal',
                                                        campaign.sponsors
                                                            .internal,
                                                        campaign,
