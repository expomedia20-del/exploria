...
export default function AccessScopesIndex({
    ...
    const activeScopes = accessScopes.filter((scope) => scope.status === 'active');
            ...
            >
                <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm lg:grid-cols-4">
                        <Stat icon={ShieldCheck} label="کل دسترسی‌ها" value={stats.total} />
                        <Stat icon={CheckCircle2} label="فعال" value={stats.active} />
                        <Stat icon={UsersRound} label="کاربران" value={stats.users} />
                        <Stat icon={Layers3} label="دسترسی سراسری" value={stats.global} />
                    </div>
                </header>
                            ...
                            {({ processing, errors }) => (
                                <>
                                    ...
                                    <div className="grid gap-2">
                                        <Label htmlFor="role_key">نقش عملیاتی</Label>
                                        <select
                                            id="role_key"
                                            name="role_key"
                                            required
                                            defaultValue={roleOptions[0]?.key ?? ''}
                                            className="h-9 rounded-md border border-input bg-background px-3 text-sm"
                                        >
                                    ...
                                    <div className="grid gap-2">
                                        <Label htmlFor="scope_type">دامنه</Label>
                                        <select
                                            id="scope_type"
                                            name="scope_type"
                                            required
                                            ...
                                            className="h-9 rounded-md border border-input bg-background px-3 text-sm"
                                        >
                                            {Object.keys(scopeOptions).map((scopeType) => (
                                    ...
                                    <div className="grid gap-2">
                                        <Label htmlFor="scope_id">شناسه محدوده</Label>
                                        <select
                                            id="scope_id"
                                            name="scope_id"
                                            defaultValue=""
                                            className="h-9 rounded-md border border-input bg-background px-3 text-sm"
                                        >
                                            <option value="">کل سیستم یا انتخاب دستی</option>
                                            {Object.entries(scopeOptions).flatMap(
                                                ([scopeType, options]) =>
                                                    options
                                                        ...
                                                        .map((option) => (
                                                            <option
                                                                ...
                                                                value={option.id ?? ''}
                                                            >
                                                                {scopeTypeLabels[scopeType] ?? scopeType}: {option.label}
                                                            </option>
                                                        )),
                                            )}
                                        </select>
                ...
                <section className="rounded-lg border border-sidebar-border/70 bg-background dark:border-sidebar-border">
                    <div className="flex items-center justify-between border-b border-sidebar-border/70 p-4 dark:border-sidebar-border">
                        <div>
                            <h2 className="font-semibold">دسترسی‌های فعال و تاریخی</h2>
                            <p className="mt-1 text-sm text-muted-foreground">
                                {activeScopes.length.toLocaleString('fa-IR')} دسترسی فعال
                            </p>
                        </div>
                                ...
                                >
                                    <div className="min-w-0">
                                        ...
                                        <p className="mt-1 text-xs text-muted-foreground">
                                            {scope.user.role ? userRoleLabels[scope.user.role] ?? scope.user.role : '-'}
                                        </p>
                                    </div>
                                    <span>{scope.roleLabel}</span>
                                    <span>{scope.scopeTypeLabel}</span>
                                    <div className="min-w-0">
                                        <p className="truncate">{scope.scopeLabel}</p>
                                        <p className="mt-1 truncate text-xs text-muted-foreground" dir="ltr">
                                    ...
                                    >
                                        ...
                                        {scope.status === 'active' ? 'فعال' : 'غیرفعال'}
                                    </span>
                                    <div>
                                        {writable && scope.status === 'active' ? (
                                            <Form
