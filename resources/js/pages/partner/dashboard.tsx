...
function describeRewardOption(option: string, step: MissionPlanStep | null) {
    ...
    if (option.includes('ارجاع') || option.includes('همراه')) {
        parts.push(
            'یعنی پاداش وقتی معنی دارد که کاربر یک نفر دیگر را همراه کند یا دعوت کند',
        );
    }
    ...
    if (option.includes('کوپن') || option.includes('تخفیف')) {
        parts.push(
            'پس پیشنهاد شما می‌تواند کوپن، تخفیف یا مزیت خرید قابل مصرف باشد',
        );
    }

    if (option.includes('امتیاز')) {
        parts.push(
            'پس پیشنهاد می‌تواند ارزش افزوده‌ای باشد که بعد از انجام گام به امتیاز یا مزیت کاربر اضافه می‌شود',
        );
    }

    if (option.includes('نشان')) {
        parts.push(
            'پس پیشنهاد بیشتر نقش یادگاری، افتخار یا مزیت نمادین برای کاربر دارد',
        );
    }

    if (option.includes('گنج')) {
        parts.push(
            'پس پیشنهاد باید حس کشف یا جایزه پنهان داشته باشد و بعد از تکمیل شرط گام فعال شود',
        );
    }
...
export default function PartnerDashboard({
    ...
    const [selectedStepIndex, setSelectedStepIndex] = useState<string>(
    ...
    const selectMissionStep = (stepIndex: string) => {
        const step = proposalContext.missionPlan.find(
            (item) => String(item.index) === stepIndex,
        );
    ...
    const tierForStep = (step: MissionPlanStep) =>
        proposalContext.rewardTiers.find(
            (tier) => tier.tierKey === step.rewardTier,
        ) ?? null;
    const actionSteps = [
        {
            title: 'تکمیل اطلاعات فروشگاه',
            description:
                'نام مسئول، موبایل، دسته‌بندی و توضیح عملیاتی را ثبت کنید تا ادمین بداند پاداش چگونه تحویل می‌شود.',
        ...
        },
        {
            ...
            description:
                'وقتی کاربر کد مصرف آورد، کد را در فرم تایید وارد کنید تا تحویل پاداش ثبت شود.',
                    ...
                    <div className="mt-4 grid gap-3 lg:grid-cols-4">
                        {actionSteps.map((step, index) => (
                            <a
                                key={step.title}
                                href={step.href}
                                className="rounded-md border border-border/70 bg-background/80 p-3 text-sm transition hover:border-primary/50"
                            >
                                ...
                                <p className="mt-2 min-h-12 text-xs text-muted-foreground">
                                    {step.description}
                                </p>
                        ...
                        {({ processing, errors }) => (
                            <>
                                ...
                                <div className="grid gap-2 md:col-span-2">
                                    ...
                                    />
                                    <InputError
                                        message={errors.operating_notes}
                                    />
                                </div>
                                <div className="flex items-center gap-2 md:col-span-2">
                                    ...
                                    <Label htmlFor="display_visibility">
                                        نمایش فروشگاه در تجربه‌های کاربر و
                                        نمایشگرها
                                    </Label>
                                    <InputError
                                        message={errors.display_visibility}
                                    />
                                </div>
                        ...
                        <div className="mb-4 rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
                            {proposalContext.campaign ? (
                                <>
                                    ...
                                    <p className="mt-1">
                                        ابتدا گام چرخه کمپین را انتخاب کنید؛ سطح
                                        پاداش همان گام به‌صورت خودکار به پیشنهاد
                                        شما وصل می‌شود.
                                    </p>
                                </>
                            ) : (
                                <p>
                                    برای ثبت پیشنهاد پاداش، ابتدا باید یک کمپین
                                    در حال تنظیم برای مکان این فروشگاه وجود
                                    داشته باشد.
                                </p>
                            )}
                        </div>
                        {proposalContext.campaign ? (
                            <div className="mb-4 rounded-md border border-sidebar-border/70 dark:border-sidebar-border">
                                <div className="border-b border-sidebar-border/70 px-3 py-2 dark:border-sidebar-border">
                                    <p className="text-sm font-medium">
                                        خلاصه چرخه و سطح‌های پاداش کمپین
                                    </p>
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        هر ردیف یک گام از مسیر کاربر است. برای
                                        ثبت پیشنهاد، روی گام مناسب کلیک کنید یا
                                        همان گام را در فرم انتخاب کنید.
                                    </p>
                                </div>
                                <div className="divide-y divide-sidebar-border/70 dark:divide-sidebar-border">
                                    ...
                                    0 ? (
                                        <p className="px-3 py-3 text-xs text-muted-foreground">
                                            چرخه مأموریت برای این کمپین هنوز
                                            آماده نشده است.
                                        </p>
                                    ) : (
                                ...
                                <>
                                    ...
                                    <div className="grid gap-2 md:col-span-2">
                                        ...
                                        <Input
                                            ...
                                            value={
                                                selectedStep?.routeIntent ??
                                                'گام چرخه را انتخاب کنید'
                                            }
                                            readOnly
                                            className="bg-muted/40"
                                        />
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            این گزینه‌ها مربوط به{' '}
                                            {selectedStep
                                                ? `گام ${selectedStep.index.toLocaleString('fa-IR')}`
                                                : 'گام انتخاب‌شده'}{' '}
                                            و سطح {selectedTier?.level ?? '-'}{' '}
                                            هستند.{' '}
                                            {describeRewardOption(
                                                selectedRewardOptionLabel,
                                                selectedStep,
                                            )}
                                        </p>
                                        <InputError
                                    ...
                                    <div className="grid gap-2">
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">
                                            عنوان را براساس گزینه/ترکیب
                                            انتخاب‌شده بنویسید؛ ادمین همین عنوان
                                            را برای بررسی پیشنهاد می‌بیند.
                                        </p>
                                        <InputError message={errors.name} />
                                    </div>
                                    <div className="grid gap-2">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            این گزینه فقط جنس پیشنهاد را مشخص
                                            می‌کند؛ گام و سطح پاداش از بالا
                                            تعیین شده است.
                                        </p>
                                        <InputError
                                    ...
                                    >
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            {reward.cycleStepIndex
                                                ? `گام ${Number(reward.cycleStepIndex).toLocaleString('fa-IR')}: ${reward.cycleStepLabel ?? '-'}`
                                                : 'گام چرخه: ثبت نشده'}{' '}
                                            · سطح: {reward.rewardTier ?? '-'}
                                            {reward.rewardOption
                                                ? ` · گزینه: ${reward.rewardOption}`
                                                : ''}
                                        </p>
                                        <p className="text-xs text-muted-foreground">
                                            صادر شده:{' '}
                                            {formatCount(
                                                reward.userRewardsCount,
                                            )}{' '}
                                            · موجودی:{' '}
