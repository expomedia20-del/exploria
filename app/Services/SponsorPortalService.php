...
class SponsorPortalService
{
    ...
    public function overview(User $user): array
    {
        ...
        $proposals = $sponsor->proposals()
            ->with(['campaign:id,code,name,status', 'preferredPartnerAccount:id,venue_id,code,name,partner_type,status', 'preferredPartnerAccount.venue:id,code,name'])
            ->latest('created_at')
            ->get()
    ...
    public function submitProposal(User $user, array $data): SponsorProposal
    {
        ...
        $campaign = ! empty($data['campaign_id'])
            ...
            : null;
        $partner = ! empty($data['preferred_partner_account_id'])
            ? PartnerAccount::query()->findOrFail($data['preferred_partner_account_id'])
            : null;
        ...
        if ($partner && $sponsor->venue_id !== null && $partner->venue_id !== $sponsor->venue_id) {
            throw ValidationException::withMessages(['preferred_partner_account_id' => 'واحد پیشنهادی به مکان این اسپانسر تعلق ندارد.']);
        }

        if ($campaign && $partner && $partner->venue_id !== $campaign->venue_id) {
            throw ValidationException::withMessages(['preferred_partner_account_id' => 'واحد پیشنهادی با مکان کمپین انتخاب‌شده هم‌خوان نیست.']);
        }

        return DB::transaction(fn (): SponsorProposal => SponsorProposal::query()->create([
