...
- فروشگاه بتواند پاداش‌های وابسته به خودش را ببیند.
- کاربر بعد از تکمیل ماموریت، پاداش دارای کد مصرف دریافت کند.
- فروشگاه مصرف کد پاداش را تایید کند.
- گزارش پایه مصرف فروشگاه قابل مشاهده باشد.
- فروشگاه بتواند پیشنهاد/تخفیف جدید ثبت کند.
- ادمین یا مدیر رواق پیشنهاد فروشگاه را تایید یا رد کند.

معیار پذیرش:

- هر فروشگاه فقط کدهای خودش را تایید کند.
...
## وضعیت اجرای Sprint 1.5

شروع اجرای Sprint 1.5 انجام شد:

- مسیر `GET /partner/dashboard` برای پنل واقعی فروشگاه/شریک اضافه شد.
- مسیرهای `GET /api/v1/partner/dashboard`, `POST /partner/redemptions/confirm`, و `POST /api/v1/partner/redemptions/confirm` اضافه شدند.
- تکمیل ماموریت وقتی پاداش به یک partner وصل باشد، برای کاربر `reward_redemption` با کد مصرف یکتا می‌سازد.
- کاربر فروشگاه فقط پاداش‌ها و کدهای مصرف مربوط به partner خودش را می‌بیند.
- فروشگاه می‌تواند کد مصرف مشتری را تایید کند؛ پس از تایید، `reward_redemptions.status` و `user_rewards.status` به وضعیت مصرف‌شده/تاییدشده می‌روند.
- تست‌های feature برای مشاهده داشبورد فروشگاه، ایجاد کد مصرف، تایید کد، جلوگیری از تایید توسط فروشگاه دیگر و گزارش API اضافه شدند.

ادامه Sprint 1.5 انجام شد:

- مسیرهای `POST /partner/offers` و `POST /api/v1/partner/offers` برای ثبت پیشنهاد/تخفیف فروشگاه اضافه شدند.
- پیشنهاد فروشگاه به صورت `reward_definition` با status `draft` و metadata تایید (`approval_status = pending_review`) ذخیره می‌شود.
- مسیرهای `POST /admin/rewards/{reward}/approve`, `POST /admin/rewards/{reward}/reject`, و نسخه API آن‌ها اضافه شدند.
